...
\newcommand{\myExpOne}{
\item Augmenting AI-assisted HPC tools to automate the code optimization workflow of OpenMP and CUDA applications via the OpenAI API.
\item Software design and engineering for distributed Kokkos tools: (1) inter-process profiling and adaptivity via PMPI and (2) job-level monitoring and feedback via LDMS.
%\item Implementing and specifying tooling API features for Kokkos Tools for OpenMP, OpenACC, MPI, and C++ standards.
\item Developed AI-assisted HPC tools using LLMs (coderosetta.com) and auto-tuning (TAU+APEX) for Kokkos applications on NVIDIA GPUs, resulting in a poster presentation at GTC 2025. 
\item Research and pathfinding on the use of AI chips (e.g., Cerebras WSE-3, Tenstorrent Wormhole) for scientific simulations. 
...
\newcommand{\myExpTwo}{
	\item Developed and maintained Kokkos Tools for the CMake and Spack build systems, tooling overheads, CI/CD, auto-tuning, and NVTX/ROCTX/VTune integration, leading to 15 merged GitHub PRs.
   % \item Contributed to autotuning features to the Kokkos 4.5 release. 
   \item Developed a debugging tool that detected 7 common Kokkos user bugs by analyzing LLVM IR of Kokkos programs via symbolic execution, leading to a paper at the SC24 Correctness Workshop. 
...
\newcommand{\myExpThree}{
  % \item Contributed to developing an LLVM OpenMP implementation, specifically the OpenMP implementation's compiler and its runtime, targetted for Department of Energy's upcoming Exascale Supercomputer platforms. 
   \item Implemented OpenMP user-defined multi-GPU scheduling for LLVM, offering a 2.1x speedup over MPI parallelization, leading to papers at IWOMP 2020 and BCB 2021.
   \item Implemented performance optimizations in LLVM for OpenMP asynchronous GPU offloading that achieved a 1.2x speedup, leading to a paper at the SC22 HiPar workshop. 
   \item Developed performance benchmarks that evaluated five major vendor OpenMP GPU implementations, leading to an ACM journal paper and an IWOMP 2021 workshop paper. 
...
\newcommand{\myExpFour}{
\item Implemented user-defined loop schedules (UDS) for OpenMP and RAJA via a prototype library for LLVM and GCC, leading to a paper at IWOMP 2018 and three GitHub PRs merged in Charm++. 
